...
so this method does NOT require an absolute ocean temperature.
"""
...
"""
    MarineShelfParams{T, M<:BMBMethod{T}}

All parameters needed to run `update!`.  The 3-D ocean interpolation
parameters present in the legacy Fortran module have been removed; it is
assumed that `T_shlf` and `S_shlf` are already interpolated to the
relevant depth of the shelf base before being passed in.
...
"""
struct MarineShelfParams{T<:AbstractFloat, M<:BMBMethod}
    bmb_method::M
...
- `tf_corr_basin`        — per-basin index correction to TF [K]
...
- `bmb_corr`             — additive per-basin correction to bmb [m/yr]
...
function MarineShelfState(
    ...
    return MarineShelfState{T, typeof(A)}(
        newfield(), newfield(), newfield(),   # T_shlf, S_shlf, dT_shlf
...
Top-level object combining parameters and state.

Construct with `MarineShelf(params, state)` or via the convenience
constructor `MarineShelf(T, nx, ny, bmb_method; kwargs...)`.
"""
struct MarineShelf{T<:AbstractFloat, M<:BMBMethod, A<:AbstractMatrix{T}}
    par::MarineShelfParams{T,M}
    now::MarineShelfState{T,A}
end

"""
    MarineShelf(T, nx, ny, bmb_method; kwargs...)

Allocate everything at once.  `kwargs` are forwarded to `MarineShelfParams`.
"""
function MarineShelf(
    ...
    return MarineShelf{T, M, Matrix{T}}(par, now)
end
